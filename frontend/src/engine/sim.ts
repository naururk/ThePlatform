...
export type Effect = { id:number; floor:number; kind:'grab'|'skim' };

export function resolveAtFloor(
  floor:number,
  players: Player[],
  sess: SessionState,
  setSess: (s:SessionState)=>void,
  update: (index:number, patch:Partial<Player>)=>void,
  pushEffect: (kind:'grab'|'skim', floor:number)=>void
){
...
export function finalizeSession(players: Player[], sess: SessionState, setSess:(s:SessionState)=>void, update:(i:number, ch:Partial<Player>)=>void){
